...
/**
 * BO-Immobilia18-specific processing.
 */
class BO_Immobilia18 extends Theme_Base {

	public
		$theme_class_slug = 'bo-immobilia18';
	...
	public function check_attachment( $attachment, $post_id ) {
		...
		} elseif ( in_array( (string) $attachment['gruppe'], array( 'PANORAMA' ) ) ) {
	...
	public function save_property_location( $post_id, $immobilie ) {
		...
		add_post_meta( $post_id, '_bo_show_prop_on_map', $this->theme_options['show_properties_on_map'] ? '1' : '0', true );
	...
	public function add_attachment_data( $att_id, $valid_image_formats, $valid_misc_formats ) {
		...
		if ( $p ) {
			...
			$panorama_images = $this->get_extended_filenames( $this->temp['panorama_images']['filenames'][$p->post_parent] );

			if ( ! empty( $panorama_images ) &&	in_array( $filename, $panorama_images, true ) ) {
	...
	public function save_attachment_data( $post_id, $immobilie ) {
		...
		if ( ! empty( $this->temp['panorama_images']['ids'][$post_id] ) ) {
			...
			foreach ( $this->temp['panorama_images']['ids'][$post_id] as $att_id ) {
				...
				$post_att_data[] = array(
					'_bo_prop_vrimg' => array(
						'url' => $img_src[0],
						'alt' => $p->post_title,
						'caption' => $p->post_title
					),
					'_bo_prop_preview' => array(
						'url' => '',
						'alt' => '',
						'caption' => ''
					)
			...
			if ( count( $post_att_data ) > 0 ) {
				add_post_meta( $post_id, 'vrimages', $post_att_data, true );
			}
	...
	public function save_agent( $post_id, $immobilie ) {
		...
		add_post_meta( $post_id, '_bo_team_profile_headline', $this->plugin->multilang_get_string_translation( $this->theme_options['agent_box_headline'] ), true );
	...
	public function do_final_processing_steps( $post_id, $immobilie ) {
		$default_values = array(
			...
			'_bo_prop_featured' => $this->theme_options['add_properties_to_slider'] ? '1' : '0'
		);
	...
	public function extend_fields( $fields ) {
		$theme_fields = array(
			...
			array(
				'name' => $this->theme_class_slug . '_add_properties_to_slider',
				'type' => 'checkbox',
				'label' => __( 'Header Slider', 'immonex-openimmo2wp' ),
				'section' => 'ext_section_' . $this->theme_class_slug . '_general',
				'args' => array(
					'description' => __( 'Show imported properties in the header slider (e.g. on front page)?', 'immonex-openimmo2wp' )
				)
			),
			array(
				'name' => $this->theme_class_slug . '_gmap_zoom',
				'type' => 'text',
				'label' => __( 'Google Map zoom', 'immonex-openimmo2wp' ),
				'section' => 'ext_section_' . $this->theme_class_slug . '_general',
				'args' => array(
					'class' => 'small-text',
			...
			array(
				'name' => $this->theme_class_slug . '_agent_box_headline',
				'type' => 'text',
				'label' => __( 'Agent box headline', 'immonex-openimmo2wp' ),
				'section' => 'ext_section_' . $this->theme_class_slug . '_general',
				'args' => array(
					'description' => __( 'Headline for the agent box on property detail pages', 'immonex-openimmo2wp' )
				)
			),
			array(
