...
/**
 * BO-HOME-specific processing.
 */
class BO_HOME extends Theme_Base {

	public
		$theme_class_slug = 'bo-home';
	...
	public function __construct( $plugin, $supported_theme_properties ) {
		...
		add_action( 'immonex_oi2wp_property_imported', array( $this, 'maybe_add_areabutler_url' ), 30, 2 );
	...
	public function maybe_convert_sold_state( $custom_field_data, $immobilie, $post_id ) {
		if (
			'bor_prop-marker' !== $custom_field_data['meta_key'] ||
	...
	public function save_attachment_data( $post_id, $immobilie ) {
		...
		if ( ! empty( $this->temp['floor_plans']['ids'][$post_id] ) ) {
			...
			if ( count( $post_att_data ) > 0 ) {
				add_post_meta( $post_id, 'floorplan_data', $post_att_data, true );
			}
	...
	/**
	 * Add property AreaButler URL if set during import.
	 *
	 * @since 5.3.5-beta
	 *
	 * @param int|string $post_id Property post ID.
	 * @param SimpleXMLElement $immobilie XML node of a property object.
	 */
	public function maybe_add_areabutler_url( $post_id, $immobilie ) {
		$immonex_ab_url = get_post_meta( $post_id, '_immonex_areabutler_url', true );
		add_post_meta( $post_id, 'bo_prop_areamapsrc', $immonex_ab_url );
	} // maybe_add_areabutler_url
