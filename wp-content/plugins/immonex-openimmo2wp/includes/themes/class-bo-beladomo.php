...
/**
 * BO-Beladomo-specific processing.
 */
class BO_Beladomo extends Theme_Base {

	public
		$theme_class_slug = 'bo-beladomo';
	...
	public function maybe_convert_sold_state( $custom_field_data, $immobilie, $post_id ) {
		if (
			'bor_prop-sale' !== $custom_field_data['meta_key'] ||
			'sold' !== $custom_field_data['meta_value']
		) {
			return $custom_field_data;
		}
		...
		if ( ! $is_sale ) {
			$custom_field_data['meta_value'] = 'rented';
		}
	...
	public function save_property_location( $post_id, $immobilie ) {
		...
		add_post_meta( $post_id, '_bo_prop_map_5', $this->theme_options['gmap_height'], true );
	...
	function maybe_convert_reference( $post_id, $immobilie ) {
		...
		if ( wp_update_post( $post ) ) {
			...
			if (
				...
				update_post_meta( $post_id, 'bor_prop-marker', '' );
			}
		}
	...
	public function extend_fields( $fields ) {
		$theme_fields = array(
			...
			array(
				'name' => $this->theme_class_slug . '_gmap_height',
				'type' => 'text',
				'label' => __( 'Google Map height', 'immonex-openimmo2wp' ),
				'section' => 'ext_section_' . $this->theme_class_slug . '_general',
				'args' => array(
					'description' => __( 'Height of Google Maps on property detail pages', 'immonex-openimmo2wp' ),
					'default_if_empty' => '410px'
				)
			),
			array(
